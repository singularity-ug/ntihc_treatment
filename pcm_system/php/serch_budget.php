...
$sql="SELECT * FROM procplanning WHERE PLANDETAILS LIKE '%$bd%' AND BUDGETSTATUS = 'RUNNING'";

$result = $conn->query($sql);
$st ='<div class="list-group"> <br />';

while($row = $result->fetch_assoc()) {
    $impt = $idxx.'__'.$row['PLANDETAILS'].'__'.$row['CODE'].'__'.$row['FINANCIALPERIOD'].'__'.$row['FIRSTQTROB']
	.'__'.$row['SECONDQTROB'].'__'.$row['THIRDQTROB'].'__'.$row['FOUTHQTROB'].'__'.$row['TOTALROB'].'__'.$row['BUDGETSTATUS'];
