...
?></b>completed </center>  </a>
   ...
   <center style="color:#000; font-weight:bold; font-size:12px;">POSTNATAL CARE</center> 
    </td> 
...
box-shadow: 0px 1px 1px 1px rgba(0, 0, 0, 0.1) !important;">
          <center> <b style=" font-size:11px;">Postnatal care</b></center> <i class=" "></i> 
...
$date = date("Y-m-d").' 00:00:00';
if ($result = mysqli_query($link, "SELECT RID, NTIHCNO FROM cmpatientsregistration WHERE TIMESTAMP >='$date'
                                    AND SERVICETYPE ='MEDICAL SERVICES' AND INSVCSTATUS ='NO' GROUP BY NTIHCNO ORDER BY NTIHCNO")) {
...
?>
  ...
  <div id="scrollable-content" style="background-color:transparent;">
   <ul>
                               <div class=""  id="medicalvistspnc" style="margin-left: -41px;"></div>
                                </div>
                                 </ul>
  </div>
      ...
      var ddt = jose.split("__")[1];
          postclient(ddt);
                $.ajax({
                       type    : 'POST',
             data    :{bb:odno},
                       url     : 'medical/medical_get_pnc.php?q='+odno,
                       success : function(data){
                                $('#dem').html(data);
                $("#demm").modal("show");
                                 }
...
<div class="col-sm-7" >
                ...
                <form class="form-horizontal" action="postnatalcare_process.php" method="post" style="height:auto;"/>  
    ...
    <thead> 
    <tr>  
              <th style="width:25%;border:0px;">Weight (kg)</th>
        <th style="width:25%;border:0px;"> <input type="text"  name="WEIGHT"  id="WEIGHT"
        required="required" style="width:100%; font-weight:normal; background-color:transparent;"> 
        </th>
    ...
    <thead> 
    <tr>  
        <th style="width:25%;border:0px;">Notes</th>
        <th style="width:75%;border:0px;"> <textarea  name="PNCDIAGNOSIS" rows="1"
         style="width:100%; font-weight:normal; background-color:transparent;"></textarea>  
        </th> 
        </tr>  </table>
    <input type="radio"  name="PANN"  id="PANN" value="No" checked="checked">Disabled
    <input type="radio"  name="PANN"  id="PANN" value="Active" >Enable
    
  <!--
     <table class="table table-striped" id="mytable" style="font-weight:normal;margin-top: -10px; "> 
     <input type="button" onClick="addRow12('mytable')" value="Add follow up vists" class="btn-success"  style="background-color:green;" > &nbsp;
 ...
 <table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;  border: 0px;text-align:left;" width="100%">
    <thead> 
    <tr> 
          <th style="width:25%;border:0px;background-color:transparent;"> </th>
    ...
    <thead> 
    <tr> 
            <td style="border:0px;width:30%;background-color:transparent;"> </td>
...
<br>
    ...
    <input type="hidden" name="REQUESTINGUNIT" id="REQUESTINGUNIT" value="POSTNATAL CARE ROOM" >
...
<Select name="ACTIONTAKEN" id="ACTIONTAKEN" required="required" style="width:100%; font-weight:normal; background-color:transparent;">
                                              <option value=""> </option>
                        <option> FAMILY PLANNING DONE</option>
    ...
    <tr> 
        ...
        <td style="width:60%;border:0px;"> <input type="date" name="FOLLOWUPDATE" id="FOLLOWUPDATE" style="width:100%;"></td>
              </tr>
        </table>
      </div> </div></div>
...
$("#impres4").modal("show");
     ...
     <script> 
          setInterval(function() {
                $.ajax({
                       type    : 'POST',
                       url     : 'medical/medicalvists_pnc.php',
                       success : function(data){
                                $('#medicalvistspnc').html(data);
                                 }
               });
               }, 2000);
