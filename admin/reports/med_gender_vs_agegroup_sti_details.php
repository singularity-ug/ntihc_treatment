...
<div class="col-sm-10">
   ...
   <a rel="facebox" title="" href="med_gender_vs_agegroup_sti_details.php?d1=0&d2=0"><button class="">
   <i class="icon-edit">  
   </i>STI's Breakdown</button> </a> 
      
   </center>  </td>   
       </tr>             
            </table>
     <br> 
   <form action="med_gender_vs_agegroup_sti_details.php" method="get">
...
NUMBER OF CLIENTS WITH STI's BETWEEN&nbsp;<?php echo $_GET['d1'] ?>&nbsp;AND&nbsp;<?php echo $_GET['d2'] ?> 
</div>
    Number of STI's
      ...
      <?php
        ...
        $result = $db->prepare("SELECT DATECREATED,MEDICALPROBLEM,  
                         ...
                         FROM clientsexamination 
                                       WHERE DATECREATED BETWEEN :a AND :b AND MEDICALTOPIC='STI' GROUP BY MEDICALPROBLEM ASC ");
  ...
  <thead>
    <tr>
      ...
      <?php
        ...
        $d1=$_GET['d1'];
        $d2=$_GET['d2'];
        $results = $db->prepare("SELECT sum(RECORDCOUNTER)  FROM clientsexamination WHERE SEX = 'FEMALE' AND DATECREATED BETWEEN :a AND :b AND MEDICALTOPIC='STI'");
        ...
        ?>
             ...
             <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
                  <?php 
        $resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE SEX = 'MALE' AND DATECREATED BETWEEN :c AND :d AND MEDICALTOPIC='STI'");
        ...
        ?>  
                  ...
                  <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
          <?php 
        $resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE DATECREATED BETWEEN :c AND :d AND MEDICALTOPIC='STI'");
        $resultia->bindParam(':c', $d1);
        $resultia->bindParam(':d', $d2);
        $resultia->execute();
        for($i=0; $cxz = $resultia->fetch(); $i++){
        ...
        ?> 
                 ...
                 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
                  <?php 
        $resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE AGE_GROUP = '10-14' AND DATECREATED BETWEEN :c AND :d AND MEDICALTOPIC='STI'");
        $resultia->bindParam(':c', $d1);
        $resultia->bindParam(':d', $d2);
        $resultia->execute();
        for($i=0; $cxz = $resultia->fetch(); $i++){
        ...
        ?>  
                 ...
                 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
                  <?php 
        $resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE AGE_GROUP = '15-19' AND DATECREATED BETWEEN :c AND :d AND MEDICALTOPIC='STI'");
        $resultia->bindParam(':c', $d1);
        $resultia->bindParam(':d', $d2);
        $resultia->execute();
        for($i=0; $cxz = $resultia->fetch(); $i++){
        ...
        ?>  
                 ...
                 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
                  <?php 
        $resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE AGE_GROUP = '20-24' AND DATECREATED BETWEEN :c AND :d AND MEDICALTOPIC='STI'");
        $resultia->bindParam(':c', $d1);
        $resultia->bindParam(':d', $d2);
        $resultia->execute();
        for($i=0; $cxz = $resultia->fetch(); $i++){
        ...
        ?>  
                ...
                <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
                  <?php 
        $resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE AGE_GROUP = '24' AND DATECREATED BETWEEN :c AND :d AND MEDICALTOPIC='STI'");
        $resultia->bindParam(':c', $d1);
        $resultia->bindParam(':d', $d2);
        $resultia->execute();
        for($i=0; $cxz = $resultia->fetch(); $i++){
