...
<div class="col-sm-10">
   ...
   <i class="icon-edit">  
     ...
     <br> 
<form action="hcg_summary.php"method="get">
...
HCT REPORT BETWEEN&nbsp;<?php echo $_GET['d1'] ?>&nbsp;AND&nbsp;<?php echo $_GET['d2'] ?> 
    </div> 
    <table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;	text-align:left;" width="100%">
		<thead> 
		<tr> 
	    <th colspan="1">SERVICE DATE</th>  
		...
		<th colspan="3" style="text-align:center;">POSITIVE</th> 
		</tr>
	...
	<tbody>
			...
			<?php
				...
				$result = $db->prepare("SELECT DATECREATED, USERINITIAL,
				                               ...
				                               sum( if( AGE_GROUP = '10-14', RECORDCOUNTER, 0 ) ) AS aa,  
                                               sum( if( AGE_GROUP = '15-19', RECORDCOUNTER, 0 ) ) AS bb, 
											   sum( if( AGE_GROUP = '20-24', RECORDCOUNTER, 0 ) ) AS cc,  
                                               sum( if( AGE_GROUP = '24', RECORDCOUNTER, 0 ) ) AS dd, 
											   ...
											   sum( if( HCGRESULTS = 'POSITIVE', RECORDCOUNTER, 0 ) ) AS pos,  
                                               sum( if( HCGRESULTS = 'NEGATIVE', RECORDCOUNTER, 0 ) ) AS neg,
											   
											   sum( if( HCGPOSITIVE = 'WANTED', RECORDCOUNTER, 0 ) ) AS wan,  
                                               sum( if( HCGPOSITIVE = 'UN WANTED', RECORDCOUNTER, 0 ) ) AS unwan     
			...
			<td  style="text-align:center;"><?php echo $row['aa']; ?></td> 
            <td  style="text-align:center;"><?php echo $row['bb']; ?></td>
			<td  style="text-align:center;"><?php echo $row['cc']; ?></td>
            <td  style="text-align:center;"><?php echo $row['dd']; ?></td> 
            <td style="text-align:center;background-color:#F9F9F9;"><?php
			$dsdsd=$row['aa'] + $row['bb']+ $row['cc']+ $row['dd'];
            ...
            <td style="text-align:center;background-color:#F9F9F9;"><?php
			$dsdsd=$row['pos'] + $row['neg'];
			echo formatMoney($dsdsd, true);
			?></td>
			
			<td  style="text-align:center;"><?php echo $row['wan']; ?></td>
			<td  style="text-align:center;"><?php echo $row['unwan']; ?></td> 
            <td style="text-align:center;background-color:#F9F9F9;"><?php
			$dsdsd=$row['wan'] + $row['unwan'];
			echo formatMoney($dsdsd, true);
			?></td>
	...
	<thead>
		<tr>
			...
			<th colspan="1" style="border-top:1px solid #999999;text-align:center;"> 
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(RECORDCOUNTER)  FROM hcg WHERE 
                ...
                <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE  
				DATECREATED BETWEEN :c AND :d AND AGE_GROUP = '15-19'");
			    ...
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE   
				DATECREATED BETWEEN :c AND :d AND AGE_GROUP = '20-24'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
			    ...
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE   
				DATECREATED BETWEEN :c AND :d AND AGE_GROUP = '24'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE DATECREATED BETWEEN :c AND :d ");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
                ...
                <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE SEX = 'FEMALE' 
                ...
                <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   	<?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE SEX = 'MALE' 
				AND DATECREATED BETWEEN :c AND :d ");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE SCHOOLING = 'IS' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   	<?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE SCHOOLING = 'OS' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE HCGRESULTS = 'POSITIVE' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   	<?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE HCGRESULTS = 'NEGATIVE' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE HCGPOSITIVE = 'WANTED' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   	<?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM hcg WHERE HCGPOSITIVE = 'UN WANTED' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
