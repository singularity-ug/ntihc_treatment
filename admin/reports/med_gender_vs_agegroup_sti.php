...
<div class="col-sm-10">
   ...
   <i class="icon-edit">  
     ...
     <br> 
   <form action="med_gender_vs_agegroup_sti.php" method="get">
...
NUMBER OF CLIENTS WITH STI's BETWEEN&nbsp;<?php echo $_GET['d1'] ?>&nbsp;AND&nbsp;<?php echo $_GET['d2'] ?> 
  ...
  END AS age_range,
				                               ...
				                               WHERE DATECREATED BETWEEN :a AND :b AND MEDICALTOPIC='STI' GROUP BY age_range ASC ");
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(RECORDCOUNTER)  FROM clientsexamination WHERE SEX = 'FEMALE' AND MEDICALTOPIC='STI' AND DATECREATED BETWEEN :a AND :b");
             ...
             <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE SEX = 'MALE' AND MEDICALTOPIC='STI' AND DATECREATED BETWEEN :c AND :d");
				...
				?> 
				 ...
				 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE SCHOOLING = 'IS' AND DATECREATED BETWEEN :c AND :d AND MEDICALTOPIC='STI'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				?> 
				 ...
				 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE SCHOOLING = 'OS' AND DATECREATED BETWEEN :c AND :d AND MEDICALTOPIC='STI'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
