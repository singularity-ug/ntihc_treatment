...
<div class="col-sm-10">
   ...
   <i class="icon-edit">  
     ...
     <br> 
<form action="counselling_art.php" method="get">
...
ART ATTENDENCE REPORT BETWEEN&nbsp;<?php echo $_GET['d1'] ?>&nbsp;AND&nbsp;<?php echo $_GET['d2'] ?> 
    </div> 
    <table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;	text-align:left;" width="100%">
		...
		<th colspan="1" style="text-align:center;">REFERRALS</th>
		</tr>
  ...
  END AS age_range,
											   ...
											   AND ARTCLINIC='REFERRED' GROUP BY age_range ASC");
	...
	<thead>
		<tr>
			...
			<th colspan="1" style="border-top:1px solid #999999;text-align:center;"> 
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(RECORDCOUNTER)  FROM laboratory WHERE SEX = 'FEMALE' AND 
				DATECREATED BETWEEN :a AND :b AND AGE_GROUP = '10-14' AND ARTCLINIC='REFERRED'");
                ...
                <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   	<?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM laboratory WHERE SEX = 'MALE' 
				AND DATECREATED BETWEEN :c AND :d AND ARTCLINIC='REFERRED'");
                ...
                <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM laboratory WHERE DATECREATED BETWEEN :c AND :d AND ARTCLINIC='REFERRED'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM laboratory WHERE SCHOOLING = 'IS' AND DATECREATED BETWEEN :c AND :d AND ARTCLINIC='REFERRED'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   	<?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM laboratory WHERE SCHOOLING = 'OS' AND DATECREATED BETWEEN :c AND :d AND ARTCLINIC='REFERRED'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM laboratory WHERE VISTTYPE = 'NEW' AND DATECREATED BETWEEN :c AND :d AND ARTCLINIC='REFERRED'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   	<?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM laboratory WHERE VISTTYPE = 'REVIST' AND DATECREATED BETWEEN :c AND :d AND ARTCLINIC='REFERRED'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM laboratory WHERE FINALREACTIVITY = 'POSITIVE' AND DATECREATED BETWEEN :c AND :d AND ARTCLINIC='REFERRED'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   	<?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM laboratory WHERE FINALREACTIVITY = 'NEGATIVE' AND DATECREATED BETWEEN :c AND :d AND ARTCLINIC='REFERRED'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				<th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   	<?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM laboratory WHERE ARTCLINIC = 'REFERRED' AND DATECREATED BETWEEN :c AND :d AND ARTCLINIC='REFERRED'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
