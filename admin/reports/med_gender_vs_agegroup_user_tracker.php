...
<div class="col-sm-10">
   ...
   <i class="icon-edit">  
     ...
     <br> 
   <form action="med_gender_vs_agegroup_user_tracker.php" method="get">
...
MEDICAL SERVICES PERFORMANCE REPORT BY BETWEEN&nbsp;<?php echo $_GET['d1'] ?>&nbsp;AND&nbsp;<?php echo $_GET['d2'] ?> 
</div>
    Number of clients served by service provider
    <table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;	text-align:left;" width="100%">
		<thead> 
		<tr> 
	    <th>SERVICE DATE</th>
        <th>SERVICE PROVIDER</th> 
        <th colspan="4" style="text-align:center;">AGE GORUP </th> 
	...
	<tbody>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$result = $db->prepare("SELECT DATECREATED, SERVICEPROVIDER,
				                               ...
				                               WHERE DATECREATED BETWEEN :a AND :b AND INSVCSTATUS='TREATMENT ROOM' GROUP BY DATECREATED,SERVICEPROVIDER ASC ");
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(RECORDCOUNTER)  FROM cmpatientsregistration WHERE 
				DATECREATED BETWEEN :a AND :b AND AGE_GROUP = '10-14' AND INSVCSTATUS='TREATMENT ROOM'");
             ...
             <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM cmpatientsregistration WHERE  
				 DATECREATED BETWEEN :c AND :d AND AGE_GROUP = '15-19' AND INSVCSTATUS='TREATMENT ROOM'");
             ...
             <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM cmpatientsregistration WHERE   
				DATECREATED BETWEEN :c AND :d AND AGE_GROUP = '20-24' AND INSVCSTATUS='TREATMENT ROOM'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				?> 
                 ...
                 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM cmpatientsregistration WHERE   
				DATECREATED BETWEEN :c AND :d AND AGE_GROUP = '24' AND INSVCSTATUS='TREATMENT ROOM'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
             ...
             <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
				...
				AND DATECREATED BETWEEN :c AND :d AND INSVCSTATUS='TREATMENT ROOM'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
             ...
             <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
				...
				?> 
                 ...
                 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM cmpatientsregistration WHERE DATECREATED BETWEEN :c AND :d AND INSVCSTATUS='TREATMENT ROOM'");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
...
<?php   
 $from_date = "$d1";
 $to_date = "$d2";
  $strQuery = "SELECT SUM(RECORDCOUNTER), SERVICEPROVIDER,
											   ...
											   AND INSVCSTATUS='TREATMENT ROOM' GROUP BY SERVICEPROVIDER ASC";
    ...
    if ($result) {
        ...
        $arrData = array(
            "chart" => array(
              "caption" => "General service performance as per service provider",
