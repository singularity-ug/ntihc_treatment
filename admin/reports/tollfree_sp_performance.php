...
<div class="col-sm-10">
 <table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;background-color:#fff; text-align:left;border: 0px;" width="100%">
   ...
   <i class="icon-edit">  
     ...
     <br> 
   <form action="tollfree_sp_performance.php" method="get">
...
<div style="font-weight:bold; text-align:center;font-size:11px;margin-bottom: 15px;">
 ...
 TOLLFREE SERVICE PERFORMANCE TRACKER BETWEEN&nbsp;<?php echo $_GET['d1'] ?>&nbsp;AND&nbsp;<?php echo $_GET['d2'] ?> 
 </div>
   Number of calls recieved by service provider as per action taken
	<table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;	text-align:left;" width="100%">
		<thead> 
		<tr> 
		<th>SERVICE PROVIDER</th>
        <th style="text-align:center;">COUNSELLED</th>
        <th style="text-align:center;">REFFERED</th>
	...
	<tbody>
			...
			<?php
				...
				$result = $db->prepare("SELECT DATECREATED, USERINITIAL,  
				                               ...
				                               sum( if( ACTIONTAKEN = 'REFER', RECORDCOUNTER, 0 ) ) AS RR,  
                                               sum( if( ACTIONTAKEN = 'COUNSEL', RECORDCOUNTER, 0 ) ) AS CC, 
											   sum( if( ACTIONTAKEN = 'BOTH', RECORDCOUNTER, 0 ) ) AS BB 
            ...
            <td  style="text-align:center;"><?php echo $row['RR']; ?></td>
			<td  style="text-align:center;"><?php echo $row['BB']; ?></td>   
            
            <td style="text-align:center;"><?php
			$dsdsd=$row['CC'] + $row['BB'] + $row['RR'];
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(RECORDCOUNTER)  FROM tollfree WHERE  ACTIONTAKEN = 'COUNSEL' AND DATECREATED BETWEEN :a AND :b");
             ...
             <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM tollfree WHERE  ACTIONTAKEN = 'REFER' AND DATECREATED BETWEEN :c AND :d");
				...
				?>  
                 ...
                 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM tollfree WHERE  ACTIONTAKEN = 'BOTH' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
...
<?php   
 ...
 $to_date = "$d2";
  $strQuery = "SELECT SUM(RECORDCOUNTER), USERINITIAL,  
											   ...
											   FROM tollfree WHERE DATECREATED >= '" . $from_date . "' AND DATECREATED <= '" . $to_date . "' 
    ...
    if ($result) {
        ...
        $arrData = array(
            "chart" => array(
              "caption" => "Tollfree services by action taken",
