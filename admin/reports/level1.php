...
<div style="font-weight:bold; text-align:center;font-size:12px;margin-bottom: 15px; font-weight:normal;">
 ...
 ATTENDECE TO ALL SERVICES BETWEEN&nbsp;<?php echo $_GET['d1'] ?>&nbsp;AND&nbsp;<?php echo $_GET['d2'] ?> 
 </div>  
   <table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;	text-align:left;" width="100%">
        ...
        <th style="text-align:center;">MATERNAL HEALTH SERVICES</th>
  ...
  END AS age_range,
                                               ...
                                               sum( if( SEX = 'MALE', RECORDCOUNTER, 0 ) ) AS MAL,
									 ...
									 sum( if( SERVICEEXTENTION = 'TREATMENT ROOM', RECORDCOUNTER, 0 ) ) AS trr, 
									 sum( if( SERVICEEXTENTION = 'COUNSELLING SERVICES', RECORDCOUNTER, 0 ) ) AS crr,  
			...
			<td  style="text-align:center;"><?php echo $row['me']; ?></td>
			<td  style="text-align:center;"><?php echo $row['co']; ?></td> 
			<td  style="text-align:center;"><?php echo $row['pre']; ?></td> 
			 
			
            <td  style="text-align:center;"><?php echo $row['FEM']; ?></td>
			<td  style="text-align:center;"><?php echo $row['MAL']; ?></td> 
            <td style="text-align:center;"><?php
			$dsdsd=$row['FEM'] + $row['MAL'];
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(RECORDCOUNTER) 
				FROM cmpatientsregistration WHERE SERVICETYPE = 'MEDICAL SERVICES' AND DATECREATED BETWEEN :a AND :b");
             ...
             <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM cmpatientsregistration 
				WHERE SERVICETYPE = 'COUNSELLING SERVICES' AND DATECREATED BETWEEN :c AND :d");
			    ...
			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM cmpatientsregistration 
				WHERE SERVICETYPE = 'PREGNANCY RELATED SERVICES' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				?> 
				 ...
				 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM cmpatientsregistration 
				WHERE SEX = 'FEMALE' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				?> 
				 ...
				 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM cmpatientsregistration 
				WHERE SEX = 'MALE' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
