...
<div class="col-sm-10">
   ...
   <i class="icon-edit">  
     ...
     <br> 
<form action="med_gender_vs_agegroup.php" method="get">
...
<div style="font-weight:bold; text-align:center;font-size:12px;margin-bottom: 15px; font-weight:normal;">
 ...
 TOTAL NUMBER OF CLIENTS VISTS BETWEEN&nbsp;<?php echo $_GET['d1'] ?>&nbsp;AND&nbsp;<?php echo $_GET['d2'] ?> 
 </div>
    Number of clients  
			...
			<?php
             ...
             <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
				...
				?> 
				 ...
				 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM cmpatientsregistration WHERE SCHOOLING = 'IS' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				?> 
				 ...
				 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM cmpatientsregistration WHERE SCHOOLING = 'OS' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
