...
<table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;border:2px solid #fff;	text-align:left;" width="100%">
   ...
   <i class="icon-edit">  
   </i>  <strong>medical database</strong></button> </a>  
   </center>  </td>   
			 </tr>             
            </table>
     <br> 
...
<th>##</th>
	...
	<th style="">SEX</th> 
    <th style="">SCHOOLING</th> <th style="">EMPLOYMENT</th>
</tr>
<?php
	$res = $MySQLiconn->query("SELECT * FROM clientsexamination ORDER BY DATECREATED asc");
	$count = $res->num_rows;
	...
	if($count > 0)
	{
		while($row=$res->fetch_array())
		{
			?>
			<tr>
			<td><input type="checkbox" name="chk[]" class="chk-box" value="<?php echo $row['EID']; ?>"  /></td>
			<td><?php echo $row['DATECREATED']; ?></td>
			<td><?php echo $row['NTIHCNO']; ?></td>
            <td><?php echo $row['VISTBY']; ?></td>
			<td><?php echo $row['MEDICALTOPIC']; ?></td>
            <td><?php echo $row['MEDICALPROBLEM']; ?></td>
			<td><?php echo $row['COUNSELLINGDONE']; ?></td>   
			<td><?php echo $row['CLIENTNAME']; ?></td> 
            <td><?php echo $row['AGE1']; ?></td>
			<td><?php echo $row['AGE_GROUP']; ?></td>
			<td><?php echo $row['SEX']; ?></td>  
            <td><?php echo $row['SCHOOLING']; ?></td> 
