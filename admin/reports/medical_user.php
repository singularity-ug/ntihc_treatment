...
<div class="col-sm-10">
   ...
   <a rel="facebox" title="" href="medical_age_group.php?d1=0&d2=0"><button class="">
   <i class="icon-edit">  
   </i> Age groups</button> </a>  
   
    <a rel="facebox" title="" href="medical_user.php?d1=0&d2=0"><button class="">
   <i class="icon-edit">  
   </i> User service performance</button> </a>  </td>  
			 </tr>             
             </table></center>
     <br> 
   <form action="medical_user.php" method="get">
...
<div style="font-weight:bold; text-align:center;font-size:11px;margin-bottom: 15px;">
 ...
 MEDICAL SERVICE PROVIDER PERFORMANCE TRACKER BETWEEN&nbsp;<?php echo $_GET['d1'] ?>&nbsp;AND&nbsp;<?php echo $_GET['d2'] ?> 
 </div>
   Number of clients served by service provider as per medical topic
	<table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;	text-align:left;" width="100%">
		<thead> 
		<tr> 
		<th>SERVICE PROVIDER</th>
        <th style="text-align:center;">GENERAL TOPIC</th>
        <th style="text-align:center;">PREGNANCY</th>
        <th style="text-align:center;">STI</th>
        <th style="text-align:center;">SGBV</th>
        <th style="text-align:center;">BODY CHANGES</th>
        <th style="text-align:center;">SEXUALHEALTH</th>
	...
	<tbody>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$result = $db->prepare("SELECT DATECREATED,MEDICALTOPIC,  USERINITIAL,  
				                               sum( if( MEDICALTOPIC = 'GENERAL', RECORDCOUNTER, 0 ) ) AS GENERAL,  
                                               sum( if( MEDICALTOPIC = 'PREGNANCY', RECORDCOUNTER, 0 ) ) AS PREGNANCY,
											   sum( if( MEDICALTOPIC = 'STI', RECORDCOUNTER, 0 ) ) AS STI, 
											   sum( if( MEDICALTOPIC = 'SGBV', RECORDCOUNTER, 0 ) ) AS SGBV,  
											   sum( if( MEDICALTOPIC = 'BODY CHANGES', RECORDCOUNTER, 0 ) ) AS BODYCHANGES,
											   sum( if( MEDICALTOPIC = 'SEXUAL HEALTH', RECORDCOUNTER, 0 ) ) AS SEXUALHEALTH 
            ...
            <td  style="text-align:center;"><?php echo $row['GENERAL']; ?> </td>
            <td  style="text-align:center;"><?php echo $row['PREGNANCY']; ?></td>
			<td  style="text-align:center;"><?php echo $row['STI']; ?></td> 
            <td  style="text-align:center;"><?php echo $row['SGBV']; ?></td>
            <td  style="text-align:center;"><?php echo $row['BODYCHANGES']; ?></td> 
            <td  style="text-align:center;"><?php echo $row['SEXUALHEALTH']; ?></td>
            
            <td style="text-align:center;"><?php
			$dsdsd=$row['GENERAL'] + $row['PREGNANCY'] + $row['STI']+ $row['SGBV'] + $row['BODYCHANGES']+ $row['SEXUALHEALTH'];
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(RECORDCOUNTER)  FROM clientsexamination WHERE MEDICALTOPIC = 'GENERAL' AND DATECREATED BETWEEN :a AND :b");
             ...
             <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE MEDICALTOPIC = 'PREGNANCY' AND DATECREATED BETWEEN :c AND :d");
				...
				?>  
                 ...
                 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE MEDICALTOPIC = 'STI' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				?>  
                 ...
                 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE MEDICALTOPIC = 'SGBV' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
				...
				?>  
                 ...
                 <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE MEDICALTOPIC = 'BODY CHANGES' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
                ...
                <th colspan="1" style="border-top:1px solid #999999;text-align:center;">
			   			    <?php 
				$resultia = $db->prepare("SELECT sum(RECORDCOUNTER) FROM clientsexamination WHERE MEDICALTOPIC = 'SEXUAL HEALTH' AND DATECREATED BETWEEN :c AND :d");
				$resultia->bindParam(':c', $d1);
				$resultia->bindParam(':d', $d2);
				$resultia->execute();
				for($i=0; $cxz = $resultia->fetch(); $i++){
 ...
 $to_date = "$d2";
  $strQuery = "SELECT SUM(RECORDCOUNTER), USERINITIAL,  
    ...
    if ($result) {
        ...
        $arrData = array(
            "chart" => array(
              "caption" => "Medical service provider(s) performance",
