...
<div style="font-weight:bold; text-align:center;font-size:14px;margin-bottom: 15px;">
 <img src="../../assets/img/logsbig.PNG" width="100%" height="30%">
     ...
     <p style="text-align:center;">Monthly clients vist(s) by gender and Age groups</p>
      <table id="example3" class="table table-table-bordered" style="font-size:9px; width:100%;">
         <thead>
         <tr>   
        <th>S/NO.</th> <th style="">FISCAL PERIOD</th>
...
$sql = "SELECT FISCALYEAR,   
			 CASE
  WHEN FISCALMONTH BETWEEN 01 AND 01 THEN 'JANUARY'
  WHEN FISCALMONTH BETWEEN 02 AND 02 THEN 'FEBRUARY'
  WHEN FISCALMONTH BETWEEN 03 AND 03 THEN 'MARCH' 
  WHEN FISCALMONTH BETWEEN 04 AND 01 THEN 'APRIL'
  WHEN FISCALMONTH BETWEEN 05 AND 02 THEN 'MAY'
  WHEN FISCALMONTH BETWEEN 06 AND 03 THEN 'JUNE' 
  WHEN FISCALMONTH BETWEEN 07 AND 01 THEN 'JULY'
  WHEN FISCALMONTH BETWEEN 08 AND 02 THEN 'AUGUST'
  WHEN FISCALMONTH BETWEEN 09 AND 03 THEN 'SEPTEMBER' 
  WHEN FISCALMONTH BETWEEN 10 AND 01 THEN 'OCTOBER'
  WHEN FISCALMONTH BETWEEN 11 AND 02 THEN 'NOVEMBER'
  WHEN FISCALMONTH BETWEEN 12 AND 03 THEN 'DECEMBER' 
  END AS month_range,	                              
				                               ...
				                               GROUP BY month_range, FISCALYEAR desc";
...
$x=1; $baTT = 0;
 while($row=$res->fetch_assoc()){
	...
	$baTT=($baTT+ intval($row['FEMALE']))+intval($row['MALE']);
  echo'<tr>'.
       '<td>'.$x.'</td>'. 
	   '<td>'.$row['month_range'].' '.$row['FISCALYEAR'].'</center></td>'.
       
	   '<td><center>'.$row['FEMALE'].'</center></td>'.
	   '<td><center>'.$row['MALE'].'</center></td>'.
	   '<td  style="background-color:#f9f9f9;"><center><B>'.$baTT.'</B></center></td>'.
		
	   '<td><center>'.$row['gpone'].'</center></td>'.
       '<td><center>'.$row['gptwo'].'</center></td>'.
	   '<td><center>'.$row['gpthree'].'</center></td>'.  
	   '<td><center>'.$row['gpfour'].'</center></td>'.  
	   '<td style="background-color:#f9f9f9;"><center><B>'.$bal.'</B></center></td>'.
   '</tr>';
$x=$x+1;
}
