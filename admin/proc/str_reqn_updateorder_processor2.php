...
if(isset($_POST['buttonsend']))
{
  ...
  $res =  $conn->query("UPDATE outgoingorders SET STOCKISSUANCESTATUS='$rb', ENTRYDATE=NOW(), ISSBY='$pb',  ISSBYDATE='$paytdd', ISSUERACCOUNT='$paygg', ISSBYNAME='$payt',
   ...
   $i =0;
    ...
    $conn->query("UPDATE recievables SET QTYISSUED = (QTYISSUED + $newadd - $oldadd) WHERE `DESCRIPTION` = '$dnx[$i]' AND `BATCHNO` ='$remdrt[$i]'");
    $conn->query("UPDATE recievables SET BALANCEINSTOCK = (BALANCEINSTOCK + $oldadd - $newadd) WHERE `DESCRIPTION` = '$dnx[$i]' AND `BATCHNO` ='$remdrt[$i]'");
    $conn->query("UPDATE items SET QTYISSUED = (QTYISSUED + $newadd - $oldadd) WHERE `DESCRIPTION` = '$dnx[$i]'");	
	}
