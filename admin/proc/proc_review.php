  ...
  <i class="fa fa-comments-o fa-5x" style="color:#00a65a;"></i>
         ...
         <B style="font-size:10px;"> &nbsp <?php echo $nameofuser; ?>  </B>
                   ...
                   <div id="collapseOne" class="panel-collapse collapse in" style="background-color: #fff;">
                       ...
                       <B>&nbsp; Orders due for confirmation of availability of funds</b><span class="sr-only">35% Complete (success)</span></div> 
                   	  </div>
             ...
             $result = mysqli_query($mysqli, "SELECT * FROM procreqn WHERE  CORACTION ='AUTHORISED' AND FORWARDTO ='$rm' AND  CONFSTATUS = 'WAITING'
                 ...
                 <h4 class="box-title">
                   <a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo">
                     Rejected procurement orders 
					  &nbsp;&nbsp;&nbsp;&nbsp <span class="label label-danger pull-right">
					   <?php
...
if ($result = mysqli_query($link, "SELECT REQUISITIONID FROM procreqn WHERE  CORACTION ='AUTHORISED' AND CONFSTATUS = 'CONFIRMED'  AND FORWARDTO ='$rm'
                                              AND AOACTION = 'REJECTED' GROUP BY REQUISITIONID ORDER BY TIMESTAMP DESC")) {        
...
?>    
             ...
             $result = mysqli_query($mysqli, "SELECT * FROM procreqn WHERE  CORACTION ='AUTHORISED' AND CONFSTATUS = 'CONFIRMED'  AND FORWARDTO ='$rm'
                                              AND AOACTION = 'REJECTED' GROUP BY REQUISITIONID ORDER BY TIMESTAMP DESC");
			 ...
			 ."***".$res['CORCOMMENT'];
              ...
              echo "<td>".$res['JOBNUMBER']."</td>";
			  echo "<td>".$res['SUBTOTAL']."</td>";
              echo '<td><a id="'.$dts.'" href="javascript:void(0);" data-toggle="tooltip" title="Reject / review"   onclick="postpetty(this.id)">
              <span class="glyphicon glyphicon-ok"> </span> </a>
              &nbsp;&nbsp;&nbsp; 
              <a id="2__'.$dts.'" href="javascript:void(0)"  onclick="getprocform(this.id)" data-toggle="tooltip" title="Click to view details"
              <span class="glyphicon glyphicon-envelope"> </span></a>
			  &nbsp;&nbsp;&nbsp;
			   <a id="3__'.$dts.'" href="javascript:void(0)"  onclick="getrejections(this.id)" data-toggle="tooltip" title="View comment" 
            ...
            <div class=""  id="getrejections" >  </div>
                <script>

          function getrejections(jose){
		  var odno = ((jose.split("__")[1]).split("***"))[1];
                $.ajax({
                       type    : 'POST',
                       url     : 'getrejections.php?q='+odno,
                       success : function(data){
                                $('#getrejections').html(data);
								$("#outx").modal("show");
                                 }
               });
               }
         ...
         <!-- /.box-body -->
           ...
           document.getElementById("myBALANCEREMAINING").value=dts[6]; 
		       ...
		       document.getElementById("myCORCOMMENT").value=dts[8]; 
           }
           </script>


             <form method="POST" action="proc_budgetreview.php">
 ...
 <tbody>
 <tr>
 <td style="width:30%;"><textarea name="CORCOMMENT" id="myCORCOMMENT"  value="" readonly="" 
...
<tr>
 ...
 <tbody>
 <tr>
 <td style="width:30%;"><textarea name="CONFCOMMENT" id="CONFCOMMENT"  value="" required="required"
 style="width:100%; text-align:left;background-color:#ecf0f5;"></textarea></td>
...
<table class="table-responsive" style="background-color:#ffffff; margin-top:-19px; ">
            <table class="table">
              <tr>
                ...
                <th style="width:75%">
				<center> <input type="radio" name="CONFSTATUS" id="CONFSTATUS" value="CONFIRMED" checked>
        <label for="CONFIRMED">CONFIRM</label>
		<input name="CONFSTATUS" type="radio" id="CONFSTATUS" value="REJECTED">
  ...
  <table class="table-responsive" style="background-color:#ffffff; margin-top:-14px; ">
        ...
        <input type="hidden" name="CONFNAME" id="CONFNAME" value="<?php echo $nameofuser; ?>" >                
        <input type="hidden" name="CONFTITLE" id="CONFTITLE" value="<?php echo $desc; ?>" >   
        <input type="hidden" name="CONFDEPT" id="CONFDEPT" value="<?php echo $dept; ?>" > 
        <input type="hidden" name="CONFPFNO" id="CONFPFNO" value="<?php echo $pf; ?>" >   
        <input type="hidden" name="CONFDATE" id="CONFDATE" value="<?php echo date('d-m-y'); ?>" >
              ...
              <div class="box-group" id="accordion"> 
                <div class="panel box box-primary">
                  <div class="box-header with-border">
                    ...
                    Approved procurement requests due for final review in presence of the approver by Order number
					&nbsp;&nbsp;&nbsp;&nbsp <span class="">
...
$result = mysqli_query($mysqli, "SELECT * FROM procreqn WHERE  CORACTION ='AUTHORISED' AND CONFEXTENSION ='$rm' 
                                              AND  AOACTION = 'APPROVED' AND PROCUREMENTSTATUS = 'WAITING'
                                              GROUP BY REQUISITIONID ORDER BY TIMESTAMP DESC ");
	...
	$dts = $res['id']."***".$res['JOBNUMBER']."***".$res['CATEGORY']."***".$res['PROCUREMENTSTATUS']."***".$res['ITEM'];
		...
		title="Click to redirect approved order to PDU" 
...
<img src="../assets/img/logsbig.PNG" width="100%" height="100%">
 ...
 <div class="modal" id="m1" tabindex="-1"    role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
           ...
           <p style="color:#ffffff; font-weight:bold; font-family:lcd; font-size:20px; text-align:center;"> Reviewed and approved procurement work orders </p>

             <table id="example7" class="table table-striped table-bordered" style="font-weight:normal;">
         <thead>
        <tr> 
                     <th>APPROVED DATE</th>
					 <th>ORDER NO.</th> 
...
$result = mysqli_query($mysqli, "SELECT * FROM procreqn WHERE  CORACTION ='AUTHORISED' AND CONFEXTENSION ='$rm' 
	...
	$dts = $res['id']."***".$res['JOBNUMBER']."***".$res['CATEGORY']."***".$res['PROCUREMENTSTATUS']."***".$res['ITEM'];
		...
		echo '<td> <a id="2__'.$dts.'" href="javascript:void(0)"  onclick="getdetailsextended(this.id)" data-toggle="tooltip" 
	...
	?>
            ...
            <div class=""  id="getdetailsextended" >  </div>
                <script>

          function getdetailsextended(jose){
		  var odno = ((jose.split("__")[1]).split("***"))[1];
                $.ajax({
                       type    : 'POST',
                       url     : 'get_followup.php?q='+odno,
                       success : function(data){
                                $('#getdetailsextended').html(data);
								$("#outx").modal("show");
                                 }
               });
               }
