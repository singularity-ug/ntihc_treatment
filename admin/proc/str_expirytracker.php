...
<div class="wrapper">
 <?php
  ...
  <!-- Full Width Column -->
  <div class="row" style="background-color:#fafcfe;">
...
function getIndicator($str123){
	...
	if($str123==''){ 		 return '<td style="background-color:#adc1c6;">'.$str123.'</td>';  	 }
	else
	{
		  $str1234=array();
          $str1234=explode('_',$tempstr);
		  $cnt = count( $str1234);
       	if($str1234[$cnt-1]==''){   	 return '<td style="background-color:#adc1c6;">'.$str123.'</td>';    	 }

		               else{     return '<td style="background-color:#f1e291;">'.$str123.'</td>'; 	}
	}
...
$sql = "SELECT * FROM `items` 
																  ...
																  WHERE DISCONTINUE = 'DISABLED' ORDER BY recievables.DESCRIPTION DESC";
 ...
 while($row=$res->fetch_assoc()){
	 ...
	 echo '<tr>'.
			     ...
			     getIndicator($row["BATCHNO"]).
                 getIndicator($row["EXPIRYDATE"]).
				 getIndicator($row["EXPIRYREMINDDATE"]).  
                 '<td>'.$row["STORELOCATION"].'</td></tr>'; 
 }
