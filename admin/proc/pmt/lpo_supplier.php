...
$stss ="";
if( $result = $connect->query("SELECT SERVICEPROVIDER FROM recievables GROUP BY SERVICEPROVIDER DESC"))
{ while ($row = $result->fetch_assoc() ) {   $stss=$stss. "<option> " . $row['SERVICEPROVIDER'] ." </option>";   }
  }
$connect->close();
?>   
     
     <br>
   <form action="lpo_supplier.php" method="get">
   
<center><strong>
Supplier <select style="width: 223px; padding:2px;" name="d1" class="tcal" value="" /> 
               <option></option>
               <?php   echo $stss; ?> 
              </select>

and name <select style="width: 223px; padding:2px;" name="d2" class="tcal" value="" />
                <option></option>
               <?php   echo $stss; ?> 
              </select>
...
<th>Local Purchase Orders supplied by &nbsp;<?php echo $_GET['d1'] ?></th><th class="hidden" <?php echo $_GET['d2'] ?> </th>
</div>
	...
	<tbody>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$result = $db->prepare("SELECT * FROM recievables WHERE SERVICEPROVIDER BETWEEN :a AND :b ORDER by id DESC ");
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(SUBTOTAL) FROM recievables WHERE SERVICEPROVIDER BETWEEN :a AND :b");
