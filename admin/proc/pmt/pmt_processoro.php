...
$sql = "INSERT INTO payments        (PAYMENTDATE,  
                             ...
                             VALUES ('$PAYMENTDATE', 
									 ...
									 mysqli_query($conn, "UPDATE `procplanning` SET `TOTALROB` = (`BUDGETAMOUNT` - '$AMOUNTPAYABLE') 
									 WHERE `PLANDETAILS` = '$PLANDETAILS'  AND `FINANCIALPERIOD` =  '$FINANCIALPERIOD' "); 
									 
									 if($QUARTERACCOUNTABLE=='1ST QUARTER'){
									 mysqli_query($conn, "UPDATE `procplanning` SET `FIRSTQTROB` = (`FIRSTQTR` - '$AMOUNTPAYABLE') 
									 WHERE `PLANDETAILS` = '$PLANDETAILS'  AND `FINANCIALPERIOD` =  '$FINANCIALPERIOD'"); }  
									 
									 if($QUARTERACCOUNTABLE=='2ND QUARTER'){
									 mysqli_query($conn, "UPDATE `procplanning` SET `SECONDQTROB` = (`SECONDQTR` - '$AMOUNTPAYABLE') 
									 WHERE `PLANDETAILS` = '$PLANDETAILS'  AND `FINANCIALPERIOD` =  '$FINANCIALPERIOD'"); }  
	 
	                                 if($QUARTERACCOUNTABLE=='3RD QUARTER'){ 
	                                 mysqli_query($conn, "UPDATE `procplanning` SET `THIRDQTROB` = (`THIRDQTR` - '$AMOUNTPAYABLE') 
									 WHERE `PLANDETAILS` = '$PLANDETAILS'  AND `FINANCIALPERIOD` =  '$FINANCIALPERIOD'");} 
									 
									 if($QUARTERACCOUNTABLE=='4TH QUARTER'){   
									 mysqli_query($conn, "UPDATE `procplanning` SET `FOUTHQTROB` = (`FOURTHQTR` - '$AMOUNTPAYABLE') 
									 WHERE `PLANDETAILS` = '$PLANDETAILS'  AND `FINANCIALPERIOD` =  '$FINANCIALPERIOD'"); }  
