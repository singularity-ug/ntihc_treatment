...
{ 
   ...
   $AOACTION = $_POST['AOACTION'];   
   $CORACTION = $_POST['CORACTION'];   
   ...
   $res =  $conn->query("UPDATE procreqn SET SENDTO='$rb', DATEREQUIRED='$pb', LOCATIONFORDELIVERY='$payt', SUBJECTOFPROCUREMENT='$gds', SUBTOTAL='$totx', CORACTION='$CORACTION',
   AOACTION='$WAITING',
   UNITOFMEASURE='$uom[$x]', 
   QUANTITY='$qty[$x]',
   ESTIMATEDCOST='$escost[$x]',
   MARKETPRICE='$marktp[$x]' 
   WHERE `REQUISITIONID` = '$on' AND DESCRIPTION='$desc[$x]'"); 
