  ...
  <i class="fa fa-comments-o fa-5x" style="color:#00a65a;"></i>
         ...
         <B style="font-size:10px;"> &nbsp <?php echo $nameofuser; ?>  </B>
                   ...
                   <div id="collapseOne" class="panel-collapse collapse in" style="background-color: #fff;">
                       ...
                       <B>&nbsp;Procurement requests due for confirmation </b><span class="sr-only">35% Complete (success)</span></div>
             ...
             $result = mysqli_query($mysqli, "SELECT *,SUM(MARKETPRICE) FROM procreqn WHERE  CORACTION ='PENDING' AND SENDTO ='$rm'
             ...
             ."***".$res['BALANCEREMAINING'];
              ...
              echo "<td>".$res['TIMESTAMP']."</td>"; 
               echo "<td>".$res['REQUISITIONID']."</td>";
			    echo "<td>".$res['SUM(MARKETPRICE)']."</td>";
...
if ($result = mysqli_query($link, "SELECT REQUISITIONID FROM procreqn WHERE  CONFSTATUS = 'REJECTED' AND SENDTO ='$rm'
                                   GROUP BY REQUISITIONID ORDER BY TIMESTAMP DESC")) {        
...
?>    
              ...
              $result = mysqli_query($mysqli, "SELECT * FROM procreqn WHERE  CONFSTATUS = 'REJECTED' AND SENDTO ='$rm'
                                   GROUP BY REQUISITIONID ORDER BY TIMESTAMP DESC");
              ...
              ."***".$res['BALANCEREMAINING'];
               ...
               echo '<td><a id="'.$dts.'" href="javascript:void(0);" data-toggle="tooltip" title="Click to notify initiator"   onclick="postpetty(this.id)">
               <span class="glyphicon glyphicon-ok"> </span> </a>
               &nbsp;&nbsp;&nbsp;
               <a id="2__'.$dts.'" href="javascript:void(0)"  onclick="getprocform(this.id)" data-toggle="tooltip" title="Click to view rejected orders"
               <span class="glyphicon glyphicon-envelope"> </span></a>
			   &nbsp;&nbsp;&nbsp;
			   <a id="3__'.$dts.'" href="javascript:void(0)"  onclick="getrejectiontwo(this.id)" data-toggle="tooltip" title="View comment" 
			  <span class="glyphicon glyphicon-remove"> </span></a> 
			   ...
			   <div class=""  id="getrejectiontwo" >  </div>
                <script>

          function getrejectiontwo(jose){
		  var odno = ((jose.split("__")[1]).split("***"))[1];
                $.ajax({
                       type    : 'POST',
                       url     : 'getrejectiontwo.php?q='+odno,
                       success : function(data){
                                $('#getrejectiontwo').html(data);
								$("#outx").modal("show");
                                 }
               });
               }
         ...
         <!-- /.box-body -->
           ...
           <div class="progress">
           <div class="progress-bar progress-bar-success" style="width: 70%;  color:#fff; text-align:left; font-weight:bold;">
            <b> Procurement budget details</b> <span class="sr-only">35% Complete (success)</span></div>
           <div class="progress-bar progress-bar-success" style="width: 10%"><span class="sr-only">20% Complete (warning)</span></div>
           <div class="progress-bar progress-bar-warning" style="width: 10%"><span class="sr-only">20% Complete (warning)</span></div>
           <div class="progress-bar progress-bar-danger" style="width: 10%"><span class="sr-only">10% Complete (danger)</span></div>
           </div>
           ...
           document.getElementById("myAPPLICANTNAME").value=dts[2]; 
...
$result = mysqli_query($mysqli, "SELECT * FROM procreqn WHERE  CORACTION ='AUTHORISED' AND SENDTO ='$rm'
                                               GROUP BY REQUISITIONID ORDER BY TIMESTAMP DESC");
