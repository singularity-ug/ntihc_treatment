...
{ 
   ...
   FUCKERCOUNTER='$FUCKERCOUNTER[$x]', 
    BALANCEINSTOCK='$bal[$x]', 
   VARIANCE='$variance[$x]', 
   BATCHNO='$batch[$x]',
   EXPIRYDATE='$expdet[$x]', 
   ...
   $i =0;
    ...
    for($i=0; $i<count($dnx);$i++){  
          $newadd = $qtyrecieved[$i]; 
		  $oldadd = $FUCKERCOUNTER[$i]; 
    $conn->query("UPDATE recievables SET QTYRECIEVED = (QTYRECIEVED + $newadd - $oldadd) WHERE `DESCRIPTION` = '$dnx[$i]' AND `BATCHNO` ='$remdrt[$i]'");
    $conn->query("UPDATE recievables SET BALANCEINSTOCK = (BALANCEINSTOCK + $newadd - $oldadd) WHERE `DESCRIPTION` = '$dnx[$i]' AND `BATCHNO` ='$remdrt[$i]'");
    $conn->query("UPDATE items SET QTYRECIEVED = (QTYRECIEVED + $newadd - $oldadd) WHERE `DESCRIPTION` = '$dnx[$i]'");	
	}
